...
public class ConditionsScreen extends Screen implements Dropdown.Host {
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
        ...
        // 展开的下拉框优先处理浮层内部滚动
        if (openDropdown != null && openDropdown.handleExpandedScroll(mouseX, mouseY, scrollY)) {
            return true;
        }
    ...
    public void render(GuiGraphics gui, int mouseX, int mouseY, float partialTick) {
        ...
        boolean popupCovering = !isSliding() && openDropdown != null && openDropdown.isExpanded()
                && !openDropdown.isMouseOver(mouseX, mouseY)
                && openDropdown.isMouseOverExpanded(mouseX, mouseY);

        if (popupCovering) {
            super.render(gui, Integer.MIN_VALUE / 2, Integer.MIN_VALUE / 2, partialTick);
        } else {
            super.render(gui, mouseX, mouseY, partialTick);
        }

        // 展开的下拉框浮层：最顶层渲染，脱离裁剪区，盖住一切
        if (!isSliding() && openDropdown != null && openDropdown.isExpanded()) {
